...
export default function CategoryPage({ params }: { params: { category: string } }) {
    ...
    useEffect(() => {
        const fetchMasterLocations = async () => {
            const response = await getMasterLocations(category);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await getEventsPaginated(searchTerm, page, size, category, selectedLocation);
                const data = response.result;
        ...
        <>
            <div className="container mx-auto px-4">
                <header className="m-8">
                    <h1 className="text-2xl font-bold mb-4 text-center capitalize">{category} Events</h1>
